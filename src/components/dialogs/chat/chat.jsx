...
const Chat = props => {
  let MessElements = props.MessData.map(Mess => (
    <Message key={Mess.id} id={Mess.id} message={Mess.message} name={Mess.name} time={Mess.time} />
  ));
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.wrapper}>{MessElements}</div>
...
const ChatForm = props => {
  ...
  return (
    <Formik
      ...
      validate={values => {
        const errors = {};
        if (values.newMessText.length >= 300) {
          errors.newMessText = 'Your message is very long';
        }
        return errors;
      }}
    >
